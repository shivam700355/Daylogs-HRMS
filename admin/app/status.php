                                    ...
                                    <table id="example1" class="table table-bordered table-striped">
                                        ...
                                        <tbody>
                                            ...
                                            while ($row = $user->fetch(PDO::FETCH_ASSOC)) {
                                                ...
                                                $status = $listing->get_status($id);

                                                ?>
                                                <tr>
                                                    <td><?php echo $i; ?></td>
                                                    <td><?php echo $row['u_name']; ?></td>
                                                    <td><?php echo isset($status['s_date']) ? $status['s_date'] : '---'; ?>
                                                    </td>
                                                    <td><?php echo isset($status['s_time']) ? $status['s_time'] : '---'; ?>
                                                    </td>

                                                    <td>
                                                        <?php
                                                        if (isset($status['s_msg'])) {
                                                            if ($status['s_msg'] === 'Not Available') {
                                                                echo '<span class="text-sm text-warning"><i class="fas fa-circle"></i> Not Available</span>';
                                                            } elseif ($status['s_msg'] === 'Working') {
                                                                echo '<span class="text-sm text-success"><i class="fas fa-circle"></i> Working</span>';
                                                            } elseif ($status['s_msg'] === 'Meeting') {
                                                                echo '<span class="text-sm text-danger"><i class="fas fa-circle"></i> Meeting</span>';
                                                            } else {
                                                                echo "<marquee style='color:#17a2b8'>{$status['s_msg']}</marquee>";
                                                            }
                                                        } else {
                                                    ...
                                                    <td>
                                                        <a href="status-report.php?u_Id=<?php echo $row['u_id']; ?>"
