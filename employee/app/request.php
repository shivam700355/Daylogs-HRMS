...
<?php
$listing = new Listing();
$request = $listing->user_request($_SESSION['u_id']);
?>
<!DOCTYPE html>
<html lang="en">
...
<body class="hold-transition sidebar-mini">
            ...
            <div class="card-tools">
              <!-- <div class="card-tools">
                <a href="#" target="_blank" class="btn btn-tbl-edit btn-xs btn-primary" data-toggle="modal"
                  data-target="#request"><i class="fa fa-plus"></i></a>
              </div> -->
              <div class="card-tools">
                <a href="add_request" class="btn btn-tbl-edit btn-xs btn-primary"><i class="fa fa-plus"></i></a>
              </div>
            </div>
          </div>
                  ...
                  <table id="example1" class="table table-bordered table-striped">
                    ...
                    <tbody>
                      ...
                      while ($row = $request->fetch(PDO::FETCH_ASSOC)) {
                        ...
                        <tr>
                          <td><?php echo $i; ?></td>
                          <td><?php echo date('j F, y', strtotime($row['r_date'])); ?></td>
                          <td><?php echo date('l', strtotime($row['r_date'])); ?></td>
                          <td><?php echo $row['r_type']; ?></td>
                          <td><?php echo $row['r_title']; ?></td>
                          <td><?php echo $row['r_action']; ?></td>
                          <td><?php echo date('d-m-Y h:i A', strtotime($row['created_at'])); ?></td>


                          <td>
                            <a href="#"
                              onclick="showRequestDetails('<?php echo $row['r_date']; ?>','<?php echo $row['r_title']; ?>','<?php echo $row['r_type']; ?>','<?php echo $row['r_action']; ?>','<?php echo $row['r_desc']; ?>')"
  ...
  <script>
    function showRequestDetails(date, title, type, action, r_desc) {
      var modalContent = document.getElementById("requestModalBody");
    ...
    <div class="row">
        <div class="col">
            <div class="form-group">
                ...
                <input type="text" class="form-control" id="requestDate" value="${date}" disabled>
            </div>
        </div>
        <div class="col">
            <div class="form-group">
                <label for="requestTitle">Title:</label>
                <input type="text" class="form-control" id="requestTitle" value="${title}" disabled>
            </div>
        </div>
    </div>
    <div class="row">
        <div class="col">
            <div class="form-group">
                ...
                <input type="text" class="form-control" id="requestType" value="${type}" disabled>
            </div>
        </div>
        <div class="col">
            <div class="form-group">
                <label for="requestAction">Action:</label>
                <input type="text" class="form-control" id="requestAction" value="${action}" disabled>
            </div>
        </div>
    </div>
    <div class="form-group">
        <label for="requestDescription">Request Description:</label>
        <textarea class="form-control" id="requestDescription" rows="3" disabled>${r_desc}</textarea>
    </div>
</form>

    `;

      $('#requestModal').modal('show'); // Show the modal using jQuery
    }
  </script>
  <!-- jQuery -->
