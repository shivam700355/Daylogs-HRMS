...
<?php
$listing = new Listing();
$activity = $listing->user_session($_SESSION["u_id"]);
?>
<!DOCTYPE html>
<html lang="en">
      ...
      <section class="content">
        <div class="card card-default">
          <div class="card-header">
            <h3 class="card-title">Active Session</h3>
                  ...
                  <table id="example1" class="table table-bordered table-striped">
                    ...
                    <tbody>
                      ...
                      ?>
                        <tr>
                          <td><?php echo $i; ?></td>
                          <td><?php echo ($_SESSION['daylogs_session'] == $row['session']) ? "Browser (Current Device)" : $row['device']; ?></td>
                          <td><?php echo $row['session']; ?></td>
                          <td><?php echo $row['created_at']; ?></td>
                          <td>
                            <button class="delete btn btn-tbl-delete btn-xs btn-danger" <?php echo ($_SESSION['daylogs_session'] == $row['session']) ? 'disabled' : ''; ?> id="<?php echo $id; ?>">
  ...
  <script type="text/javascript" language="javascript">
    $(document).ready(function() {
      ...
      $('.delete').click(function() {
        if (confirm('This action will delete this session. Are you sure?')) {
          var s_id = this.id;
          // AJAX request
          $.ajax({
            url: 'action/delete',
